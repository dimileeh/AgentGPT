...
class AgentDB:
    ...
    async def get_artifact(self, artifact_id: str) -> Artifact:
        if self.debug_enabled:
            LOG.debug(f"Getting artifact with an artifact_id: {artifact_id}")
        try:
            with self.Session() as session:
                if (
                    ...
                    LOG.error(f"Artifact not found with an artifact_id: {artifact_id}")
    ...
    async def get_artifact_by_file_name(self, task_id: str, file_name: str) -> Artifact:
        if self.debug_enabled:
            LOG.debug(f"Getting artifact by file_name: {file_name}")
        try:
            with self.Session() as session:
                if (
                    artifact_model := session.query(ArtifactModel)
                    ...
                    LOG.info(f"Artifact not found with a file_name: {file_name}")
                    return None
        except SQLAlchemyError as e:
            LOG.error(f"SQLAlchemy error while getting artifact: {e}")
            raise
